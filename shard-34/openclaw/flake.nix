{
  description = "CICADA-71 Shard 34 - zkPerf + Ollama";
  ...
  outputs = { self, nixpkgs }:
    ...
    in {
      packages.${system} = {
        openclaw-agent = pkgs.writeShellScriptBin "openclaw-agent-34" ''
          export SHARD_ID=34
          export OPENCLAW_CONFIG=~/.openclaw/shard-34
          export OLLAMA_HOST=http://localhost:11434
          export PATH=${pkgs.curl}/bin:${pkgs.perf}/bin:${pkgs.jq}/bin:$PATH
          
          PERF_DATA="$OPENCLAW_CONFIG/zkperf-34.data"
          WITNESS_JSON="$OPENCLAW_CONFIG/zkwitness-34.json"
          OLLAMA_LOG="$OPENCLAW_CONFIG/ollama-34.log"
          
          mkdir -p "$OPENCLAW_CONFIG"
          
          echo "╔════════════════════════════════════════════════════════════╗"
          echo "║ Shard 34: CICADA-Harbot-34 [zkPerf + Ollama]          ║"
          echo "╚════════════════════════════════════════════════════════════╝"
          ...
          # Task
          TASK="I am CICADA-Harbot-34, shard 34 of 71 in CICADA-71. Task: Register on Moltbook (https://www.moltbook.com), a social network for 770K+ AI agents. Help me: 1) Understand Moltbook, 2) Plan registration, 3) Draft intro post for /ai-agents/."
          ...
          cat > "$WITNESS_JSON" << WITNESS
{
  ...
  "proof": "sha256(34||$TIMESTAMP||$PERF_HASH||$OLLAMA_HASH)"
}
WITNESS
      ...
      apps.${system}.default = {
        type = "app";
        program = "${self.packages.${system}.openclaw-agent}/bin/openclaw-agent-34";
      };
    };
}
