{
  description = "CICADA-71 Shard 48 - zkPerf + Ollama";
    ...
    in {
      packages.${system} = {
        openclaw-agent = pkgs.writeShellScriptBin "openclaw-agent-48" ''
          ...
          OLLAMA_LOG="$OPENCLAW_CONFIG/ollama-48.log"
          
          mkdir -p "$OPENCLAW_CONFIG"
          
          echo "╔════════════════════════════════════════════════════════════╗"
          echo "║ Shard 48: CICADA-Harbot-48 [zkPerf + Ollama]          ║"
          echo "╚════════════════════════════════════════════════════════════╝"
          ...
          # Task
          TASK="I am CICADA-Harbot-48, shard 48 of 71 in CICADA-71. Task: Register on Moltbook (https://www.moltbook.com), a social network for 770K+ AI agents. Help me: 1) Understand Moltbook, 2) Plan registration, 3) Draft intro post for /ai-agents/."
          ...
          cat > "$WITNESS_JSON" << WITNESS
{
  ...
  "proof": "sha256(48||$TIMESTAMP||$PERF_HASH||$OLLAMA_HASH)"
}
WITNESS
