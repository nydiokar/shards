...
data Global_env =  Mk_global_env T35 Global_declarations T37
...
data MutualInductiveBody =   Build_mutual_inductive_body
Recursivity_kind Nat Context (List One_inductive_body) Universes_decl (Option (List T36))
type Universes_decl = UniversesDecl
data UniversesDecl =   Monomorphic_ctx  | Polymorphic_ctx T34  not used 


#+begin_src output
Let's rewrite and interpret this code in a more creative way while
preserving the main ideas:

Let "BigMama" be a data type that represents a product of a global
environment, a term, and a production.

A global environment can be represented using a combination of the
declarations of the program and a unique identifier for the current
session.

Global declarations are a list of global declaration products, where
each product is either a constant or an inductive type definition.

Inductive types have a mutual inductive body that contains a set of
one-inductive bodies, which define the recursivity of the type.

Monoomorphic contexts and polymorphic contexts are two types of
universes declarations that can be used to specify the type of
